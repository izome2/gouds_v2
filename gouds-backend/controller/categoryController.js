...
const getShowingCategory = async (req, res) => {
  try {
    const categories = await Category.find({ status: "show" }).sort({ sortOrder: 1, _id: -1 });

    const categoryList = readyToParentAndChildrenCategory(categories);

    // Find the Ramadan category
    const ramadanIndex = categoryList[0].children.findIndex(
      (category) => category.name.en === "Ramadan"
    );

    if (ramadanIndex !== -1) {
      const [ramadanCategory] = categoryList[0].children.splice(ramadanIndex, 1);
      categoryList[0].children.unshift(ramadanCategory);
    }

    const eidIndex = categoryList[0].children.findIndex(
      (category) => category.name.en === "Eid box"
    );

    if (eidIndex !== -1) {
      const [eidCategory] = categoryList[0].children.splice(eidIndex, 1);
      categoryList[0].children.unshift(eidCategory);
    }
  ...
  try {
    ...
    if (category) {
      ...
      if (req.body.labels !== undefined) {
        category.labels = req.body.labels;
      }
...
// New function to update category labels
const updateCategoryLabels = async (req, res) => {
  try {
    ...
    res.status(200).send({
      message: "Category labels updated successfully!",
    });
  } catch (err) {
    res.status(500).send({
      message: err.message,
